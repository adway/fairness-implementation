...
nsduh <- read_tsv("src/research/fairness-implementation/data/NSDUH_2018_Tab.tsv")
youth <- nsduh %>% 
  subset(CATAG2 == 1)
...
youth_split <- initial_split(youth_recoded)
...
youth_rec <- youth_rec %>% 
  update_role(names(youth_recoded[length(names(youth)):length(names(youth_recoded))-1]), new_role = "predictor") %>%
  update_role(SUD, new_role = "outcome") %>%
  step_rm(RSKYFQDGR, RSKYFQTES)
youth_rec  
